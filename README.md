# 🏥 Loop AI: Omnichannel Voice Agent for Hospital Networks
...
```text
├── app/
│   ├── main.py       # FastAPI application, routing, and Twilio/Web endpoints
│   ├── graph.py      # LangGraph state machine, intent router, and LLM nodes
│   └── tools.py      # Automated CSV-to-SQLite database initialization
├── data/
│   ├── hospitals.csv # Raw hospital dataset (provided)
│   └── hospitals.db  # Auto-generated SQLite database
├── frontend/
│   ├── index.html    # Minimalist Web UI
│   └── app.js        # MediaRecorder audio streaming logic
